...
@Controller
@RequestMapping(value="/tjssc")
public class TjsscController {
	
	@Resource
	private TjsscServiceI service;
	...
	private String getCurrentNum(HttpServletRequest request){
        ...
        List<TjsscData> list =  service.getCurrentNum(selectDay);
        JsonArray array = new JsonArray();
        String lastNo = "";
        for(int i=0;i<list.size();i++){
        	TjsscData data = list.get(i);
	...
	private String getBzList(HttpServletRequest request){
        ...
        List<TjsscData> list =  service.getBzList(currentDay);
        JsonArray array = new JsonArray();
        for(int i=0;i<list.size();i++){
        	TjsscData data = list.get(i);
    		JsonObject json = new JsonObject();
